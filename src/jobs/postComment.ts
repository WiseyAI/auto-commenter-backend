...
export const postComment = async (postUrn: string, commentText: string, accessToken: string) => {
    try {
        ...
        if (!userResponse.ok) {
            ...
            throw new Error(`Failed to fetch user profile: ${userResponse.status} ${userError}`);
        }
